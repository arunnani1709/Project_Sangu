...
const MedicalCertificate = () => {
  ...
  const addHeaderFooter = (doc) => {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("Dr.Patil's Navajeevana ayurveda chikitsa kendra ", 105, 20, { align: "center" });
      doc.setFontSize(10);
      doc.text(
      ...
      doc.text("Sri Devi Ayurveda Clinic", 105, 270, { align: "center" });

      doc.setFontSize(9);
      doc.setTextColor(0, 0, 0);
      doc.text(
        "Dr.Patil's Navajeevana Chikitsa Kendra Veerapura ",
        105,
        275,
        { align: "center" }
      );
      doc.setTextColor(0, 0, 255);
      doc.text("Mob: 9535261996 | Email: Sangu@gmail.com", 105, 280, { align: "center" });
    }
  };
...
This is to certify that patient Mr./Ms. ${form.applicantName}, aged about ${form.age} years, was diagnosed as ${form.disease}, and was under my treatment from ${form.fromDate} to ${form.toDate}. He/She required rest for ${form.restDays} days from ${form.restFromDate} to ${form.restToDate}, so concerned authorities do need full.
    ...
    y += 10;
    doc.text(`Registration No: ${form.registrationNo}`, pageWidth - marginLeft - 80, y);

    addHeaderFooter(doc);

    const pdfBlob = doc.output("blob");
    const formData = new FormData();
    formData.append("certificate", pdfBlob, `Medical_Certificate_${form.applicantName}.pdf`);

    try {
      await axios.post("http://localhost:3001/api/upload/certificate", formData);
  ...
  <div className="space-y-8">
    ...
    <p className="text-center mt-6 text-gray-700 font-medium">Thanking You</p>
