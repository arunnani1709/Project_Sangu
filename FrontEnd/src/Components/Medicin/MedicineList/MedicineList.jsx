...
const MedicineList = () => {
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        const res = await axios.get("/api/medicines"); // proxy in vite will forward to backend
        setMedicines(res.data); // backend should return an array
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const filtered = medicines
      .filter((med) => {
        ...
        const isLow =
          threshold ? med.quantity < threshold && med.quantity > 0 : false;
  ...
  // ✅ Handle delete (backend + frontend update)
        ...
        {[
          {
            ...
            inactive: "bg-white border border-green-500 text-green-700",
          },
          {
            key: "low",
            label: "Getting Out of Stock",
            active: "bg-yellow-500 text-white",
            inactive: "bg-white border border-yellow-400 text-yellow-600",
          },
          {
            key: "out",
            label: "Out of Stock",
            active: "bg-red-600 text-white",
            inactive: "bg-white border border-red-500 text-red-600",
          },
          ...
          <table className="min-w-full table-auto text-sm sm:text-base text-left">
            <thead className="bg-green-100 text-green-800">
              <tr>
                ...
                <th className="px-3 sm:px-6 py-3 border-b text-center whitespace-nowrap">
                  Action
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredMedicines.length > 0 ? (
                filteredMedicines.map((med, index) => (
                  <tr
                    key={med.id || med.code}
                    className={`border-b ${getRowClass(med)}`}
                  >
