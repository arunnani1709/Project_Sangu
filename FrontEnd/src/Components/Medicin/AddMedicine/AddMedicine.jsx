...
if (!name.trim() || !code.trim() || quantity === "" || !type.trim()) {
  ...
  // ✅ Helper for gram-based medicines
  const isGramBased = medicine.type === 'Powder' || medicine.type === 'Basma';

  return (
    <div className="max-w-xl w-full mx-auto mt-10 bg-white p-2 rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative">
        ...
        <div>
          <label className="block text-sm text-gray-700 mb-1">
            Quantity {isGramBased && '(in grams)'}
          </label>
          <input
            type="number"
            name="quantity"
            placeholder={
              isGramBased ? 'Enter quantity in grams' : 'Quantity to Add'
            }
            required
            min="0"
